...
function TheoryPanel({ lesson, section }) {
  ...
  return (
    <div style={{ padding: '20px 24px', overflowY: 'auto', height: '100%' }}>
      ...
      {notes && (
        ...
        >
          ...
          <div style={{ flex: '1 1 220px', minWidth: 220 }}>
            ...
            <div
              style={{
                borderLeft: '2px solid rgba(240,201,41,0.8)',
      ...
      <div style={{ padding: '4px 0 0', marginBottom: 20 }}>
        {lines.map((line, i) => {
          ...
          if (line.startsWith('- **')) {
            ...
            return (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: 13, color: '#e6edf3', lineHeight: 1.65 }}>
                  {parts.map((p, pi) =>
                    pi % 2 === 1 ? (
                      <strong key={pi} style={{ color: section.color }}>
                        {p}
                      </strong>
                    ) : (
                      p
          ...
          const processed = line
            .replace(
              /`([^`]+)`/g,
              (_, code) =>
                `<code style="background:#11161d;color:#00ff88;padding:1px 5px;border-radius:3px;font-family:'JetBrains Mono',monospace;font-size:12px">${code}</code>`
            )
            .replace(/\*\*([^*]+)\*\*/g, (_, text) => `<strong style="color:#e6edf3">${text}</strong>`);

          return (
            <p
              key={i}
              style={{ fontSize: 13, color: '#aab3bd', lineHeight: 1.75, marginBottom: 8 }}
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {showConfetti && (
        <Confetti
          recycle={false}
          numberOfPieces={300}
          colors={['#00ff88', '#00d4ff', '#ff6b35', '#bf5af2', '#f0c929']}
        />
      )}
          ...
          {tab === 'code' && (
            <div style={{ flex: 1, display: 'flex', gap: 0, overflow: 'hidden' }}>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid #21262d' }}>
                <CodeRunner code={code} onSuccess={handleSuccess} showChallenge={showChallenge} challenge={lesson.challenge} />
              </div>
              ...
              <div style={{ width: 280, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#080b0f' }}>
                <div style={{ padding: '10px 12px', borderBottom: '1px solid #21262d' }}>
                  <motion.button
                    ...
                    style={{
                      width: '100%',
                      background: showChallenge ? 'rgba(255,107,53,0.12)' : '#11161d',
                      border: `1px solid ${showChallenge ? '#ff6b35' : '#21262d'}`,
                      color: showChallenge ? '#ff6b35' : '#8b949e',
                      padding: '6px 12px',
                      borderRadius: 4,
                      fontSize: 11,
                      fontWeight: 600,
          ...
          {tab === 'timeline' && (
            <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
              ...
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 12, color: '#8b949e', marginBottom: 12, lineHeight: 1.7 }}>
                  The Event Loop is what makes JS non-blocking. It watches the call stack - when it is empty,
